...
final class AppDatabase {
    ...
    private init() {
        do {
            ...
            // The key is persisted now so it survives future upgrades to encrypted storage.
            ...
            AppLogger.shared.log("Database initialized securely at \(databaseURL.path) using Keychain Key.", category: .database, level: .info)
        } catch {
            AppLogger.shared.log("Database Error: \(error.localizedDescription)", category: .database, level: .error)
